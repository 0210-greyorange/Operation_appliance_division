...
import time
import math
from parts_seg import CleanU_Net as parts_seg
from albumentations.pytorch.transforms import img_to_tensor
from binary_seg_model import CleanU_Net as binary_seg
...
def splitFrames(videoFileName, path):  # 视频地址，图像保存地址,把视频抽取为图像进行分割
    ...
    while success:
        ...
        if (i % timeF == 0):  # 按一秒一帧提取图像
            ...
            # plt.imshow(data)
            # plt.show()
            cv2.imwrite(path + "/" + str(j) + ".jpg", data)
...
def save_video(path, path_save, h, w):
    fps = 1

    size = (h, w)  # 获取图片宽高度信息
    print(size)
    fourcc = cv2.VideoWriter_fourcc(*"MJPG")
    videoWrite = cv2.VideoWriter(path_save + "/" + "test.avi", fourcc, fps,  # 视频保存地址
                                 size)  # 根据图片的大小，创建写入对象 （文件名，支持的编码器，5帧，视频大小（图片大小））
    # videoWrite = cv2.VideoWriter('0.mp4',fourcc,fps,(1920,1080))

    files = os.listdir(path)  # 图像地址
    out_num = len(files)
    for i in range(out_num):
        fileName = path + "/" + str(i).zfill(4) + '.png'  # 循环读取所有的图片,假设以数字顺序命名
...
if __name__ == '__main__':
    ...
    # **********
    types = "image"
    path = r"Z:\Endo17\data\cropped_train\instrument_dataset_7\images"  # 要分割的图像或者视频或者nii所在的文件夹地址
    path_save = r"Z:\Endo2018\test3"  # 要保存图像的地址
    ...
    if types == "image":
        ...
        save_video(path_save, path_save, h, w)  # 可以播放保存的视频展示分割结果
